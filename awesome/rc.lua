...
beautiful.border_focus = 0
awful.spawn.with_shell("picom -b --log-level=warn")
awful.spawn.with_shell("sh ~/.config/awesome/scripts/start_polybar.sh")
...
local function set_wallpaper(s)
    -- Wallpaper
    beautiful.wallpaper = "/home/onex/Descargas/4w8jxyi56if41.jpg"
...
globalkeys = gears.table.join(
    awful.key({ modkey }, "1", function ()
        ...
        awful.tag.viewonly(sc.tags[1])  
    end),
    awful.key({ modkey }, "2", function () 
        local sc = screen[1]
        awful.screen.focus(sc)
        awful.tag.viewonly(sc.tags[2])
    end),
    awful.key({ modkey }, "3", function ()
        local sc = screen[1]
        awful.screen.focus(sc)
        awful.tag.viewonly(sc.tags[3])
    end),
    awful.key({ modkey }, "4", function ()
        local sc = screen[1]
        awful.screen.focus(sc)
        awful.tag.viewonly(sc.tags[4])
    end),
    awful.key({ modkey }, "5", function ()
        local sc = screen[1]
        awful.screen.focus(sc)
        awful.tag.viewonly(sc.tags[5])
    end),
    awful.key({ modkey }, "6", function () 
        local sc = screen[1]
        awful.screen.focus(sc)
        awful.tag.viewonly(sc.tags[6]) 
    end),
    ...
    awful.key({ }, "Print", function() awful.spawn("flameshot gui") end,
              {description = "show flameshot gui", group = "launcher"})
)
...
for i = 1, 9 do
    globalkeys = gears.table.join(globalkeys,
        ...
        awful.key({ modkey, "Shift" }, "#" .. i + 9,
			      ...
			      if i >= 7 and i <= 9 then
			          tag = client.focus.screen.tags[i-6]
			      end
		          end
                          if tag then
...
awful.rules.rules = {
    ...
    -- Rule for bluetoothctl module floating window
    { rule = { class = "FloatingBluetooth" },
      properties = {
	 floating = true,
	 placement = awful.placement.centered -- Opcional: la centra en la pantalla
      }
    },
}
-- }}}
